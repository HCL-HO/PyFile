package com.hec.app.framework.http;
...
public class OkHttpClientManager {
    ...
    private static final String AUTHENTICATION_KEY = "X-Hec-Authentication";
    private static OkHttpClientManager mInstance;
    private OkHttpClient mOkHttpClient;
    private OkHttpClient trendHistoryClient;
    ...
    private OkHttpClientManager() {
        ...
        //cookie enabled
        mOkHttpClient.setCookieHandler(BaseApp.cookieManager);
        ...
        trendHistoryClient = new OkHttpClient();
        //cookie enabled
        //trendHistoryClient.setCookieHandler(new CookieManager(null, CookiePolicy.ACCEPT_ORIGINAL_SERVER));
        trendHistoryClient.setConnectTimeout(60, TimeUnit.SECONDS);
        /*just for test !!!*/
        trendHistoryClient.setHostnameVerifier(new HostnameVerifier() {
            @Override
            public boolean verify(String hostname, SSLSession session) {
                return true;
            }
    ...
    public static void setHeaders(String authKey, String userID, String userName) {
//        TestUtil.print("req: "+ userName + "+" + BaseService.SALT + "+" + req);
        getInstance().authKey = authKey;
        getInstance().userID = userID;
        getInstance().userName = userName;
        getInstance().systemVersion = Build.VERSION.RELEASE;
        getInstance().deviceName = android.os.Build.MODEL;
        getInstance().versionCode = BaseApp.instance().getVersionCode();
        getInstance().os = "Android";
        getInstance().password = StringUtil.getMd5Hash(authKey + "12qwaszx" + userID);
    }

    public static void setHeaders(String connection){
        getInstance().connection = connection;
    }

    private void buildHeaders(Request.Builder builder) {
        if (authKey != null && authKey.length() > 0) {
            builder.addHeader(AUTHENTICATION_KEY, authKey);
        }
        ...
        if(systemVersion != null){
            builder.addHeader("sysVersion", systemVersion);
        }
        if(deviceName != null){
            builder.addHeader("deviceName", deviceName);
        }
        if(versionCode != null){
            builder.addHeader("versionCode", versionCode);
        }
        if(os != null){
    ...
    private void buildHeaders(Request.Builder builder, String verifyKey) {
        if (authKey != null && authKey.length() > 0) {
            builder.addHeader(AUTHENTICATION_KEY, authKey);
        }
        if (userID != null && userID.length() > 0) {
        ...
        if(verifyKey != null){
            TestUtil.print("verifyKey:" + verifyKey);
            builder.addHeader("verifyKey", verifyKey);
        }
        if(os != null){
            builder.addHeader("OS", os);
        }
     ...
     * ============Get方便的访问方式============
     */
    public static Response get(String url, String verifyKey) throws IOException {
        return getInstance().getGetDelegate().get(url, verifyKey);
    }
     ...
     * ============POST方便的访问方式===============
     */
    public static Response post(String url, String bodyString, String verifyKey) throws IOException {
        return getInstance().getPostDelegate().post(url, bodyString, verifyKey);
    }

    public static Response post(String url, String bodyStr, Boolean isForm) throws IOException {
        return getInstance().getPostDelegate().post(url, bodyStr, isForm);
    }
    ...
    public class PostDelegate {
         ...
         */
        public Response post(String url, String bodyStr, String verifyKey) throws IOException {
            return post(url, bodyStr, null, verifyKey);
        }

        //        public Response post(String url, String bodyStr, Boolean isForm) throws IOException {
...
//        }
        ...
        public Response post(String url, String bodyStr, Object tag, String verifyKey) throws IOException {
            //RequestBody body = RequestBody.create(MEDIA_TYPE_STRING, bodyStr);
            RequestBody body = RequestBody.create(MEDIA_TYPE_JSON, bodyStr);
            Request request = buildPostRequest(url, body, tag, verifyKey);
            Log.i("headers","head heart:"+request.headers().toString()+ " " +url);
            Response response = mOkHttpClient.newCall(request).execute();
            return response;
        }
        ...
        private Request buildPostRequest(String url, RequestBody body, Object tag, String verifyKey) {
            Request.Builder builder = new Request.Builder()
                    .url(url)
                    .post(body);
            buildHeaders(builder, verifyKey);
            if (tag != null) {
                builder.tag(tag);
            }
            Request request = builder.build();
    ...
    public class GetDelegate {
        private Request buildGetRequest(String url, Object tag, String verifyKey) {
            Request.Builder builder = new Request.Builder()
                    .url(url);
            buildHeaders(builder, verifyKey);
            if (tag != null) {
        ...
        public Response get(Request request) throws IOException {
            if(request != null && request.urlString().contains("trend")){
                Log.i("wxj","request " + request.urlString());
                Call call = trendHistoryClient.newCall(request);
         ...
         * 同步的Get请求
         */
        public Response get(String url, String verifyKey) throws IOException {
            return get(url, null, verifyKey);
        }
        ...
        public Response get(String url, Object tag, String verifyKey) throws IOException {
            final Request request = buildGetRequest(url, tag, verifyKey);
            Log.i("headers","head: "+request.headers().toString() + " " + url);
            return get(request);
        }
